...
3.  **Cash (Instant)**: `Bonus + Coupon - Service Charge - Commission`.
4.  **Cash (15-Day/Co)**: `Bonus + Coupon - Item Total - Service Charge`.
5.  **Card (Instant)**: `Payout + Bonus + Item Total - Commission - Service Charge`.
6.  **Card (15-Day/Co)**: `Payout + Bonus`.
""")
...
def calculate_order_payout(row, cash_co_ids):
    ...
    if coupon == 0:
        coupon = float(row.get('Discount Amount', 0) or 0)
    ...
    if is_card: 
        if is_instant:
            # Case 5a: Card + Instant
            # Driver Pays Resto (Item-Comm)
            # Balance = Payout + Bonus + (Item - Comm) - Svc
    ...
    if df_list:
        ...
        if uploaded_cash_co:
            ...
            if df_cash is not None:
                ...
                if id_col:
                    cash_co_ids = set(df_cash[id_col].astype(str).str.strip())
        ...
        if uploaded_advance:
            ...
            if df_adv is not None:
                ...
                if 'driver phone' in acm and 'avance' in acm:
                    ...
                    adv_g.rename(columns={'Avance': 'Advance Amount'}, inplace=True)
        ...
        if sel:
            ...
            # Updated columns to show Calculation details
            show_cols = ['order id', 'order day', 'restaurant name', 'Resto Type', 'Payment Method', 'status', 'item total', 'driver payout', 'Bonus Amount', 'coupon discount', 'Calculation Type', 'Calculated Payout']
